...
for SSGF_ind = 1:nSSGF
    ...
    for R_ind = 1:3
        ...
        th(R_ind) = title(R_titles{R_ind},'interpreter','latex','fontsize',30);
    end
...
for i = 1:3
    figure(i)
    set(gca,'fontsize',20)
    set(th(i),'fontsize',30)
    xlabel('$U_{10}$ [m/s]','interpreter','latex')
    drawnow
    set(gcf,'position',[28    23   752   470],'color','w')
end
% add legend
figure(2)
lh = legend([h(1) h(2) h(3)]);
    set(lh,'fontsize',20,'interpreter','latex','location','southeast');
...
print(sprintf('imgs/mass_flux_%s',v(i)),'-dpdf')
end
